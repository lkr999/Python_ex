...
pdf.cell(w=80, h=ch, txt='Feature 1', border=1, ln=0, align='C')
pdf.cell(w=80, h=ch, txt='Feature 2', border=1, ln=1, align='C')
# Table contents
pdf.set_font('Arial', '', 16)
for i in range(len(df)):
    pdf.cell(w=80, h=ch, txt=df['feature 1'].iloc[i], border=1, ln=0, align='C')
    pdf.cell(w=80, h=ch, txt=df['feature 2'].iloc[i].astype(str), border=1, ln=1, align='C')
...
pdf.set_font('Arial', '', 16)
pdf.cell(w=30, h=ch, txt="Date: ", ln=0)
pdf.cell(w=30, h=ch, txt="04/11/2024", ln=1)
pdf.cell(w=30, h=ch, txt="Written by: Lee", ln=1)
pdf.ln(ch)
# pdf.multi_cell(w=0, h=5, txt=lorem.paragraph())
pdf.image('example_chart.png', x = 10, y = None, w = 100, h = 0, type = 'PNG', link = '') # 차트 이미지 입력
pdf.ln(ch)
